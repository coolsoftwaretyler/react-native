...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps
{
  ...
  // `accessibilityContainer` - MUST be handled before `accessible` so the setter override works
  if (oldViewProps.accessibilityContainer != newViewProps.accessibilityContainer) {
    ...
    NSLog(@"[AccessibilityContainer] Setting _isAccessibilityContainer = %d", _isAccessibilityContainer);
...
- (BOOL)isAccessibilityElement
{
  ...
  if (_isAccessibilityContainer) {
    NSLog(@"[AccessibilityContainer] isAccessibilityElement called, returning NO for container");
    return NO;
  }
...
- (void)setIsAccessibilityElement:(BOOL)isAccessibilityElement
{
  ...
  if (_isAccessibilityContainer) {
    NSLog(@"[AccessibilityContainer] Ignoring setIsAccessibilityElement:%d because this is a container", isAccessibilityElement);
...
{
  NSLog(@"[AccessibilityContainer] accessibilityElements called, _isAccessibilityContainer = %d", _isAccessibilityContainer);
  if (!_isAccessibilityContainer) {
    return [super accessibilityElements];
  }
  [self updateAccessibilityElementsIfNeeded];
  NSLog(@"[AccessibilityContainer] Returning %lu accessibility elements", (unsigned long)_accessibilityElements.count);
...
- (void)updateAccessibilityElementsIfNeeded
{
  if (_accessibilityElements != nil) {
    NSLog(@"[AccessibilityContainer] Using cached elements");
    return;
  }

  NSLog(@"[AccessibilityContainer] Building accessibility elements array");
  NSMutableArray *elements = [NSMutableArray new];

  // First, add an accessibility element for the container itself
  // This allows VoiceOver to access the container's action (e.g., the outer Pressable's onPress)
  if (super.accessibilityLabel || super.accessibilityTraits != UIAccessibilityTraitNone) {
    ...
    NSLog(@"[AccessibilityContainer] Added container element with label: %@", super.accessibilityLabel);
  }

  // Then collect all accessible child elements
  UIView *container = self.contentView ?: self;
  NSLog(@"[AccessibilityContainer] Scanning %lu subviews", (unsigned long)container.subviews.count);

  for (UIView *subview in container.subviews) {
    NSLog(@"[AccessibilityContainer] Checking subview: %@, isAccessibilityElement: %d", [subview class], subview.isAccessibilityElement);
    if (subview.isAccessibilityElement) {
      [elements addObject:subview];
      NSLog(@"[AccessibilityContainer] Added subview as accessibility element");
      ...
      NSLog(@"[AccessibilityContainer] Added %lu elements from subview's accessibilityElements", (unsigned long)subElements.count);
      ...
      NSLog(@"[AccessibilityContainer] Subview has %ld accessibility elements", (long)count);
      for (NSInteger i = 0; i < count; i++) {
        id element = [subview accessibilityElementAtIndex:i];
        if (element) {
          [elements addObject:element];
          NSLog(@"[AccessibilityContainer] Added element %ld from container", (long)i);
        }
      }
    }
  }
  _accessibilityElements = [elements copy];
  NSLog(@"[AccessibilityContainer] Final element count: %lu", (unsigned long)_accessibilityElements.count);
}
